@echo off
	...
	set new="!now! | %time:~0,5% | !pocet! | !five! | !ten! | !mimo! | !empty! | !first! | !sezonka! | !final! |-|  !sum[8]! | !sum[9]! | !sum[10]! | !sum[11]! | !sum[12]! | !sum[13]! | !sum[14]! | !sum[15]! | !sum[16]!"
	set /a finsez=%final%-%sezonka%
	@echo !pocet!>c:\temp\pocet.txt
	set /a rozdiel=%pocet%-%aktual%
	%windir%\system32\ftp.exe -s:"C:\temp\ftp.bat"
	@echo !old!>c:\temp\new.txt
	@echo !new!>> c:\temp\new.txt
	if exist "c:\temp\log\%today%.txt"	(
					@echo !new!>> c:\temp\log\%today%.txt
					)
	set h=%time:~0,1%
	if !h!==1 (
			set hod=%time:~0,2%
	...
	set sum[%hod%]=""!sum[%hod%]!""
	echo prvovstupov pocet -  5m  -  15m -  m.c. - empty - prvy - 20+ - final  
	echo prvovstupov %pocet%  -  %five%    -  %ten%  -  %mimo%  -   %empty%   -  %first%  -  %sezonka% - "" %final% "" - %finsez%
	echo ------------------------------------ 
	echo ------------------------------------ 
...
::	echo kontrola %control% %control2%
	echo predtym !aktual! , teraz !pocet! - !sezonka! = !finsez! , pribudlo !rozdiel!
	timeout /t 20